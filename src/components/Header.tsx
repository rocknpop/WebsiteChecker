...
export default function Header({ currentPath, onNavigate }: HeaderProps) {
  ...
  return (
    <>
      <header
        id="site-header"
        className={`fixed top-0 left-0 right-0 z-40 border-b border-gray-100 ${
          isMobile ? "" : "transition-all duration-300"
        } ${scrolled ? "bg-white/95 backdrop-blur-lg shadow-md" : "bg-white/80 backdrop-blur-lg shadow-sm"}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div
              ...
              id="logo-wrap"
            >
              <div className={`h-9 w-9 bg-gradient-to-tr from-blue-600 via-indigo-600 to-cyan-500 rounded-xl flex items-center justify-center shadow-md shadow-indigo-500/20 ${isMobile ? "" : "group-hover:scale-105 transition-transform duration-150"}`}>
                ...
                return (
                  <div key={link.path} className="relative group/nav">
                    <button
                      ...
                      className={`relative group px-4 py-2 rounded-full text-sm transition-colors duration-150 cursor-pointer ${
                        active
                    ...
                    >
                      {!active && link.gradient ? (
                        <span className={`bg-gradient-to-r ${link.gradient} bg-clip-text group-hover:text-transparent transition-colors duration-150`}>
                          {link.name}
                        </span>
                      ) : (
                        link.name
                      )}
                      {/* Dot indicator for active link — no pulse on mobile */}
                      {active && (
                        <span className={`absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-blue-600 rounded-full${isMobile ? "" : " animate-pulse"}`} />
                      )}
                    </button>

                    {/* Tooltip — desktop only */}
                    {link.tooltip && (
                      <div className="hidden md:block absolute top-full left-1/2 -translate-x-1/2 mt-2 px-3 py-1.5 bg-gray-900 text-white text-[11px] font-medium rounded-lg whitespace-nowrap opacity-0 group-hover/nav:opacity-100 pointer-events-none transition-opacity duration-150 z-50 shadow-lg">
                        {link.tooltip}
            ...
            <div className="flex items-center space-x-3">
              <button
                onClick={() => handleLinkClick("/")}
                className={`hidden sm:flex items-center space-x-1.5 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-sm font-bold rounded-full shadow-md active:scale-95 transition-colors duration-150 ${isMobile ? "" : "hover:from-blue-700 hover:to-indigo-700 hover:shadow-lg hover:scale-105"}`}
              >
                <Sparkles className={`w-3.5 h-3.5${isMobile ? "" : " animate-pulse"}`} />
      ...
      <div
        className={`fixed inset-0 z-50 md:hidden ${isMobile ? "" : "transition-opacity duration-300"} ${
      ...
      >
        ...
        {/* Drawer */}
        <div
          className={`absolute inset-y-0 right-0 w-72 bg-white shadow-2xl flex flex-col transform ${isMobile ? "" : "transition-transform duration-300 ease-in-out"} ${
