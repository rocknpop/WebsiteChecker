// Standalone Vercel function for GET /api/blog-posts/:slug. Data inlined — see the
// comment at the top of api/analyze-decision.ts for why a shared api/_lib/*.ts import
// isn't used here: Vercel's builder failed to bundle those shared files correctly, and
// the earlier version of this function that imported from ../_lib/blog-data never
// executed in production (requests fell through to the index.html rewrite).
